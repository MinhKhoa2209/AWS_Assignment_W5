...
export API=$(aws cloudformation describe-stacks \
  --stack-name dmk-node-express --region us-west-2 \
...
# OR Console → CloudWatch → /aws/lambda/dmk-node-express → latest stream
```
...
```bash
sam delete --stack-name dmk-node-express --region us-west-2
```
